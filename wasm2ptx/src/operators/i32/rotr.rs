...
pub fn handle_i32_rotr(
    memory_manager: &mut MemoryManager,
    stack: &mut Stack,
    entry_point: &mut PTXEntryPoint,
) { 
    let (rotation_amount, rotation_type) = stack.pop().expect("Stack underflow during I32Rotr");
    let (value, value_type) = stack.pop().expect("Stack underflow during I32Rotr");
    ...
    if let Some((result_reg, result_type)) = memory_manager.new_register(value_type) {
        ...
        panic!("Failed to allocate a new register for I32Rotr operation");
    }
}