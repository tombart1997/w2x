...
pub fn handle_i32_sub(
    memory_manager: &mut MemoryManager,
    stack: &mut Stack,
    entry_point: &mut PTXEntryPoint,
) { 
    let (right, right_type) = stack.pop().expect("Stack underflow during I32Sub");
    let (left, left_type) = stack.pop().expect("Stack underflow during I32Sub");
    ...
    let (result_reg, reg_type) = memory_manager
        ...
        .expect("Failed to allocate register for I32Sub result");

    entry_point.add_instruction(PTXInstruction::Sub {
