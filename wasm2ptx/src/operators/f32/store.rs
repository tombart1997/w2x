...
pub fn handle_f32_store(
    ...
    let (value, value_type) = stack.pop().expect("Stack underflow during F32Store (value)");
    let (address, address_type) = stack.pop().expect("Stack underflow during F32Store (address)");
    ...
    // Always upcast address to U64, not F64! Addresses must be integers in PTX
