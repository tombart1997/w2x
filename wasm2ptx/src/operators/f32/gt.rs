...
/// Handles the WebAssembly F32Gt (floating-point greater-than) operator.
...
pub fn handle_f32_gt(
    memory_manager: &mut MemoryManager,
    stack: &mut Stack,
    entry_point: &mut PTXEntryPoint,
) {  
    let (right, right_type) = stack.pop().expect("Stack underflow during F32Gt");
    let (left, left_type) = stack.pop().expect("Stack underflow during F32Gt");
    ...
    if let Some((pred_reg, reg_type)) = memory_manager.new_predicate_register() {
        ...
        panic!("Failed to allocate predicate register for F32Gt");
    }
}